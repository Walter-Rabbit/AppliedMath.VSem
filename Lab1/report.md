...
Задача линейного программирования в произвольной (общей) форме - задача, в которой требуется минимизировать или максимизировать линейную форму при заданных ограничениях (как равенствах, так и неравенствах)ю
...
Для данной системы строится симплекс таблица и делаются такие же преобразования, как и в при обычным решением симплекс методом. Цель - свети переменные r к 0, когда это произойдёт их можно исключить из таблицы и продолжать решение уже без "искусственных" переменных.
...
### **5. Может ли ОДР в задаче ЛП состоять из одной удинственной точки? Если да, то привести пример.**
...
В случае если в задаче размерность $\overline x$ равна двум на плоскости строится область допустимых решений. Потом строится прямая, отвечающая значению функции F равно нолю и вектор-градиент, составленный из коэффициентов целевой функции. Будем параллельно двигать прямую до первого или последнего касания ОДР (в случае с минимизацией последнее, в случае максимизации - последнее). Угловая точка, в которой будет пересечение - минимальное решение.
